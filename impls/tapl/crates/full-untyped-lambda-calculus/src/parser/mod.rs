...
mod tests {
    ...
    fn test_parser_multiple_import() {
        let (commands, _) = parser::parse(r#"
        use "std"
        use "bazinga"
        "#).expect("");
        
        assert_eq!(commands, [Command::Import("std".into()), Command::Import("bazinga".into())]);
    }
    ...
    fn test_bind() {
        ...
        assert_eq!(commands, [Command::Bind(FileInfo::default(), Binding::NameBind)]);
